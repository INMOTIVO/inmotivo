...
const PropertiesGrid = () => {
    ...
    queryFn: async () => {
      ...
      if (userLocation && data) {
        const propertiesWithDistance = data.map(property => {
          const distance = calculateDistance(userLocation.lat, userLocation.lng, Number(property.latitude), Number(property.longitude));
          return {
            ...property,
            distance
          };
  ...
  return <section id="propiedades" className="py-8 md:py-12 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-6 space-y-2">
          <h2 className="text-2xl md:text-4xl font-bold">
            Propiedades <span className="text-primary">destacadas</span>
          </h2>
          <p className="text-base md:text-lg text-muted-foreground">Explora las mejores opciones disponibles en tu zona</p>
        </div>

        {isLoading ? <div className="text-center py-12">
            <p className="text-muted-foreground">Cargando propiedades...</p>
          </div> : properties && properties.length > 0 ? <Carousel opts={{
        align: "start",
        loop: true
