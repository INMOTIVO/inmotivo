...
const NearbyProperties = ({ filters, searchCriteria }: NearbyPropertiesProps) => {
  ...
  return (
    <>
      {/* Propiedades cercanas - Más pequeño en móvil */}
      <div className="absolute top-4 left-4 z-[1000] w-[160px] md:w-[280px] lg:w-[320px]">
        <Card className="p-1 md:p-4 bg-background/95 backdrop-blur shadow-lg">
          <div className="flex items-center justify-between mb-1 md:mb-3">
            <h3 className="font-semibold text-[11px] md:text-lg leading-tight">Propiedades<br className="md:hidden" />cercanas</h3>
            {selectedProperties.length > 0 && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => setShowSelected(true)}
                className="text-[9px] md:text-sm h-5 md:h-8 px-1.5 md:px-3"
              >
                ({selectedProperties.length})
              </Button>
            )}
          </div>
          <div className="space-y-0.5 md:space-y-3 max-h-[calc(100vh-12rem)] md:max-h-[calc(100vh-16rem)] overflow-y-auto pr-0.5 md:pr-1">
            {isLoading || !properties || properties.length === 0 ? (
              <div className="text-center py-2 md:py-8">
                <p className="text-[9px] md:text-sm text-muted-foreground">
                  {isLoading ? 'Buscando...' : 'No hay'}
                </p>
              </div>
            ) : (
              ...
              return (
                <Card
                  key={property.id}
                  className={`p-1 md:p-3 cursor-pointer transition-colors ${
                  ...
                  <div className="flex flex-col md:flex-row gap-1 md:gap-3">
                    {images[0] && (
                      <img
                        src={images[0]}
                        alt={property.title}
                        className="w-full h-16 md:w-24 md:h-24 object-cover rounded flex-shrink-0"
                      />
                    )}
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-[9px] md:text-sm lg:text-base truncate leading-tight">{property.title}</h4>
                      <p className="text-primary font-semibold text-[9px] md:text-sm lg:text-base">{priceFormatted}</p>
                      <div className="flex items-center gap-0.5 md:gap-1 text-[8px] md:text-xs text-muted-foreground mt-0.5">
                      ...
                      <Button
                        ...
                        className="mt-0.5 md:mt-2 w-full text-[8px] md:text-sm h-4 md:h-8 py-0"
                      >
                        Ver
                      </Button>
                    </div>
